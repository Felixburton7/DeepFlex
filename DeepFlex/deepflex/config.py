# /home/s_felix/DeepFlex/deepflex/config.py

"""
Configuration handling for the DeepFlex ML pipeline.
...
def get_env_var_config() -> Dict[str, Any]:
    ...
    Environment variables should be prefixed with DEEPFLEX_ and use
    underscore separators for nested keys.

    Examples:
        DEEPFLEX_PATHS_DATA_DIR=/path/to/data
        DEEPFLEX_MODELS_RANDOM_FOREST_N_ESTIMATORS=200

    Returns:
        Dict containing configuration from environment variables
    """
    config = {}
    prefix = "DEEPFLEX_" # Changed prefix
...
def load_config(
    ...
    Args:
        ...
        use_env_vars: Whether to use environment variables (DEEPFLEX_*).
    ...
    try:
        ...
        if isinstance(numeric_level, int):
            ...
            logging.getLogger("deepflex").setLevel(numeric_level) # Set for our package too
...
def validate_config(config: Dict[str, Any]) -> None:
    """
    Perform basic validation of the merged DeepFlex configuration.
    ...
    if "{temperature}" in dataset_cfg["file_pattern"]:
        logger.warning(f"Found '{{temperature}}' in 'dataset.file_pattern'. This is likely incorrect for DeepFlex aggregated data.")
...
def get_output_dir(config: Dict[str, Any]) -> str:
    ...
    """
    return config.get("paths", {}).get("output_dir", "./output/deepflex/") # Provide default
...
def get_models_dir(config: Dict[str, Any]) -> str:
    ...
    """
    return config.get("paths", {}).get("models_dir", "./models/deepflex/") # Provide default
