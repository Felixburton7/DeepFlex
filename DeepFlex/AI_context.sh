...
OUTPUT_FILE="$PWD/DeepFlex_context.txt" # Renamed output file

# Ensure script runs from the project root directory
if [ ! -f "pyproject.toml" ] || [ ! -d "deepflex" ]; then
    echo "Error: This script must be run from the DeepFlex project root directory."
    exit 1
fi

{
    # Header
    echo "=========================================================="
    echo "          DeepFlex: Temperature-Aware Protein Flexibility ML Pipeline" # Renamed
    echo "=========================================================="
    ...
    echo "=========================================================="
    echo "DeepFlex expects a SINGLE aggregated CSV file (e.g., 'aggregated_rmsf_data.csv') specified in the config."
    ...
    echo "=========================================================="
    echo "Usage Examples (DeepFlex)"
    echo "=========================================================="
    echo "# Train the unified model (uses aggregated data specified in config)"
    echo "deepflex train"
    echo ""
    echo "# Train a specific model type (e.g., random_forest)"
    echo "deepflex train --model random_forest"
    echo ""
    echo "# Evaluate the trained model (uses test split from aggregated data)"
    echo "deepflex evaluate --model random_forest"
    echo ""
    echo "# Generate predictions using the trained model REQUIRES temperature"
    echo "deepflex predict --input new_proteins_features.csv --temperature 310.5"
    echo ""
    echo "# Compare model's predictions across a range of temperatures"
    echo "deepflex compare-temperatures --input test_features.csv --temp-range 300,400,5 --output-dir ./temp_comparison"
    echo ""
    echo "# Use OmniFlex mode (if model was trained with it)"
    echo "deepflex train --mode omniflex"
    echo ""
    echo "# Override a config parameter"
    echo "deepflex train --param models.neural_network.training.epochs=50"
    echo ""
    ...
    echo "---------------------------------------------------------"
    for file in deepflex/__init__.py deepflex/config.py deepflex/pipeline.py deepflex/cli.py; do # Updated path
        if [ -f "$file" ]; then
            echo "===== FILE: $file ====="
            cat "$file"
            echo ""
    ...
    echo "### Model Files ###"
    echo "---------------------------------------------------------"
    for file in deepflex/models/__init__.py deepflex/models/base.py deepflex/models/random_forest.py deepflex/models/neural_network.py; do # Updated path
        if [ -f "$file" ]; then
            echo "===== FILE: $file ====="
            cat "$file"
            echo ""
    ...
    echo "---------------------------------------------------------"
    for file in deepflex/data/__init__.py deepflex/data/loader.py deepflex/data/processor.py; do # Updated path
        if [ -f "$file" ]; then
            echo "===== FILE: $file ====="
            cat "$file"
            echo ""
    ...
    echo "---------------------------------------------------------"
    for file in deepflex/temperature/__init__.py deepflex/temperature/comparison.py; do # Updated path
        if [ -f "$file" ]; then
            echo "===== FILE: $file ====="
            cat "$file"
            echo ""
    ...
    echo "### Utility Files ###"
    echo "---------------------------------------------------------"
    for file in deepflex/utils/__init__.py deepflex/utils/helpers.py deepflex/utils/metrics.py deepflex/utils/visualization.py; do # Updated path
        if [ -f "$file" ]; then
            echo "===== FILE: $file ====="
            cat "$file"
            echo ""
    ...
    OUTPUT_DATA_DIR="./output/deepflex" # Updated path
    ...
    # Footer
    echo "=========================================================="
    echo "End of DeepFlex Context Document" # Renamed
    echo "=========================================================="

} > "$OUTPUT_FILE"

echo "DeepFlex context file created at: $OUTPUT_FILE"