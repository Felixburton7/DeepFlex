# /home/s_felix/DeepFlex/deepflex/cli.py

"""
Command-line interface for the DeepFlex ML pipeline.
...
import pandas as pd
import numpy as np
from deepflex.utils.helpers import ensure_dir



# Updated imports for DeepFlex structure
from deepflex.config import (
    load_config,
    get_enabled_models,
...
from deepflex.pipeline import Pipeline
from deepflex.models import get_available_models # Keep this for listing
...
@click.group()
@click.version_option(package_name="deepflex", version="0.1.0") # Updated package name
def cli():
    """
    DeepFlex: ML pipeline for temperature-aware protein flexibility prediction.
...
def train(
    ...
    Examples:
        deepflex train
        deepflex train --model random_forest
        deepflex train --mode standard
        deepflex train --input path/to/my_aggregated_data.csv
        deepflex train --param models.neural_network.training.epochs=50
    """
    ...
    if not model_list:
        ...
        sys.exit(1)
    if len(model_list) > 1:
         click.echo(click.style(f"Warning: Multiple models enabled/specified ({model_list}). DeepFlex trains one unified model. Using first: '{model_list[0]}'", fg="yellow"))
...
# Place this corrected command function in deepflex/cli.py
...
def compare_temperatures(model, config, param, input, temp_list, temp_range, output_dir, mode):
    ...
    try:
        ...
        from deepflex.data.processor import load_and_process_data, prepare_data_for_model
        ...
        try:
            ...
            from deepflex.utils.visualization import save_plot # Assume this exists
...
def preprocess(input, config, param, output, mode):
    ...
    """
    from deepflex.data.processor import load_and_process_data # Import here
...
def run(model, config, param, input, mode, skip_analysis):
    ...
    if not model_list:
        ...
        sys.exit(1)
    if len(model_list) > 1:
         click.echo(click.style(f"Warning: Multiple models enabled/specified ({model_list}). DeepFlex runs one unified model. Using first: '{model_list[0]}'", fg="yellow"))
         model_list = [model_list[0]]
...
def list_models():
    ...
    Examples:
        deepflex list-models
    """
    models = get_available_models() # Function imported from deepflex.models
