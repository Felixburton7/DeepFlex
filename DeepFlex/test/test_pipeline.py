...
"""
Test the DeepFlex pipeline end-to-end using aggregated data.
...
# Add project root to path to import deepflex
project_root = Path(__file__).resolve().parent.parent
sys.path.insert(0, str(project_root))

# Check if imports work before proceeding
try:
    from deepflex.config import load_config, get_enabled_models
    from deepflex.pipeline import Pipeline
    from deepflex.data.processor import load_and_process_data, prepare_data_for_model # For direct check
    ...
    print("Ensure DeepFlex is installed correctly (`pip install -e .`) and tests are run from the project root.")
    sys.exit(1)

class TestPipeline(unittest.TestCase):
    """Test the DeepFlex pipeline with aggregated data."""
    ...
    def setUpClass(cls):
        ...
        cls.output_dir = Path(cls.temp_dir) / 'output' / 'deepflex' # Match default structure
        cls.models_dir = Path(cls.temp_dir) / 'models' / 'deepflex'
        ...
        cls.config['paths']['output_dir'] = str(cls.output_dir.parent) # Pipeline expects parent of deepflex subdir? Check Pipeline init. Let's assume it expects the base output dir.
